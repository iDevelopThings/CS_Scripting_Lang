...
val isUnderDarkTheme: Boolean
    get() {
        val lookAndFeel = LafManager.getInstance().currentUIThemeLookAndFeel 
        return lookAndFeel?.isDark == true
    }
/**
...
fun invokeLaterOnWriteThread(action: () -> Unit) {
...
val Project.libraryTable
    get() = LibraryTablesRegistrar.getInstance().getLibraryTable(this)
fun LibraryTable.getOrCreateLibrary(name: String): Library =
    getLibraryByName(name) ?: createLibrary(name)
...
// StubIndex.getInstance().getAllKeys(VoltumNamedElementIndex.KEY, ctx.project)
inline fun <Key : Any, reified Psi : PsiElement> getAllKeys(
    indexKey: StubIndexKey<Key, Psi>,
    project: Project
): Collection<Key> =
...
inline fun <Key : Any, reified Psi : PsiElement> getAllValues(
    indexKey: StubIndexKey<Key, Psi>,
    project: Project
): Collection<Psi> =
    StubIndex.getInstance().getAllKeys(indexKey, project).flatMap {
        getElements(indexKey, it, project, null)
    }

inline fun <Key : Any, reified Psi : PsiElement> getAllKeyValues(
    indexKey: StubIndexKey<Key, Psi>,
    project: Project
): Collection<Pair<Key, Psi>> =
    StubIndex.getInstance().getAllKeys(indexKey, project).flatMap { key ->
        getElements(indexKey, key, project, null).map { key to it }
    }
...
inline fun <R> Project.nonBlocking(crossinline block: () -> R, crossinline uiContinuation: (R) -> Unit) {
    if (isUnitTestMode) {
        ...
        ReadAction.nonBlocking(Callable {
            ...
            .inSmartMode(this)
            .expireWith(VoltumPluginDisposable.getInstance(this))
...
@Service
class VoltumPluginDisposable : Disposable {
    companion object {
        @JvmStatic
        fun getInstance(project: Project): Disposable = project.service<VoltumPluginDisposable>()
    }
...
inline fun <reified TOther : PsiElement> PsiElement.IsTypeOrEquals(other: PsiElement): Boolean {
    if (this == other)
        return true
