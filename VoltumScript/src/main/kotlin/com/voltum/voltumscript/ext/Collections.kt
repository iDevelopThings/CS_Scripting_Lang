...
enum class ObservableModificationType {
    ADD, REMOVE, UPDATE
}
data class ObservableModificationEvent<T>(
    val type: ObservableModificationType,
    val element: T,
    val index: Int
) {
    companion object {
        fun <T> add(element: T, index: Int) = ObservableModificationEvent(ObservableModificationType.ADD, element, index)
        fun <T> remove(element: T, index: Int) = ObservableModificationEvent(ObservableModificationType.REMOVE, element, index)
        fun <T> update(element: T, index: Int) = ObservableModificationEvent(ObservableModificationType.UPDATE, element, index)
    }
}
class ObservableCollection<T>(
    val onChange: (MutableCollection<T>, ObservableModificationEvent<T>) -> Unit,
    val decorated: MutableCollection<T> = mutableListOf()
) : MutableCollection<T> by decorated {

    override fun add(e: T) =
        decorated.add(e).also {
            if (it) onChange(this, ObservableModificationEvent.add(e, decorated.size - 1))
        }

    override fun addAll(collection: Collection<T>) =
        decorated.addAll(collection).also {
            if (it) onChange(this, ObservableModificationEvent.add(collection.last(), decorated.size - collection.size))
        }

    override fun remove(element: T) =
        decorated.remove(element).also {
            if (it) onChange(this, ObservableModificationEvent.remove(element, decorated.size))
        }

    override fun removeAll(elements: Collection<T>) =
        decorated.removeAll(elements).also {
            if (it) onChange(this, ObservableModificationEvent.remove(elements.last(), decorated.size))
        }

    override fun retainAll(elements: Collection<T>) =
        decorated.retainAll(elements).also {
            if (it) onChange(this, ObservableModificationEvent.remove(elements.last(), decorated.size))
        }

    override fun clear() {
        decorated.clear()
        onChange(this, ObservableModificationEvent.remove(decorated.last(), decorated.size))
    }
